...
そのスクリプトを `sudo` 付きで実行しても、最初にパスワードを求められるため、解決しません
...
ただし、`gohat` は危険なため、乗っ取られる予定のある方は使っちゃダメです
...
1回目の起動時に何をしているかというと、`gohat` が `gohat` 自身にSUIDを付けています
...
ディスク上にただ存在するだけでも、それを知られると悪用される可能性があります
